...
ANaveNodriza2::ANaveNodriza2()
{
	...
	UGameplayStatics::GetAllActorsOfClass(GetWorld(), ANaveNodriza2::StaticClass(), Instances);
	
	if (Instances.Num() > 1)
	{

		Instance = Cast<ANaveNodriza2>(Instances[0]);
...
void ANaveNodriza2::BeginPlay()
{
	Super::BeginPlay();


	GetWorldTimerManager().SetTimer(MemberTimerHandle, this, &ANaveNodriza2::FireR, 5.0f, true, 0.0f);//hara la funcion de disparo cada tiempo determinado.


}

void ANaveNodriza2::Tick(float DeltaSeconds)
{
...
void ANaveNodriza2::FireShotR(FVector FireDirectionR)
{
	...
	if (bCanFire == true)
	{
		if (FireDirectionR.SizeSquared() > 0.0f)
		{
			...
			bCanFire = false;
			World->GetTimerManager().SetTimer(TimerHandle_ShotTimerExpired, this, &ANaveNodriza2::ShotTimerExpired, FireRate);//funcion que hace desaparecer la bala en un tiempo
...
void ANaveNodriza2::ShotTimerExpired()
{
...
void ANaveNodriza2::NotifyHit(UPrimitiveComponent* MyComp, AActor* Other, UPrimitiveComponent* OtherComp, bool bSelfMoved, FVector HitLocation, FVector HitNormal, FVector NormalImpulse, const FHitResult& Hit)
{
